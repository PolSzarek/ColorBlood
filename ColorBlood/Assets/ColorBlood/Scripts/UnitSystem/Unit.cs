...
public class Unit : MonoBehaviour
{
	...
	private v_AIController template;
	...
	private void Start()
	{
		// template = Instantiate<v_AIController>(template); //we copy the template otherwise it's going to overwrite the original asset!
	...
	private IEnumerator DealAttack()
	{
		while(targetOfAttack != null)
		{
			...
			yield return new WaitForSeconds(1f / template.attackRotationSpeed);
	...
	private void SufferAttack(int damage)
	{
		...
		template.currentHealth -= damage;

		if(template.currentHealth <= 0)
		{
			template.currentHealth = 0;
			Die();
		}
	}
